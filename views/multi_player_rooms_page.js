...
export default class MultiPlayerRoomPage extends React.Component {
  constructor(props){
   ...
   //console.log(this.props.navigation.getParam("username", "Jura"));
    this.state = {rooms:[],timer: null,counter: 0};
  ...
  componentDidMount(){
    let timer = setInterval(this.reloadRooms.bind(this), 4000);
    this.setState({timer});
  ...
  render() {
    ...
    return (
    <ScrollView onScroll = {this.reloadRooms.bind(this)} contentContainerStyle={styles.contentContainer} overScrollMode = 'always'>
        {productList}
